...
Object::Object(const glm::vec3& position, const glm::vec3& rotation,
    ...
    : _position(position), _rotation(rotation), _scale(scale), _pMesh(pMesh) {
}
...
const Vertex::Descriptor ObjectVertex::getDescriptor() const {
    ...
    attributeDescriptors[1].offset = offsetof(ObjectVertex, model) + 4 * sizeof(float);
    ...
    attributeDescriptors[2].offset = offsetof(ObjectVertex, model) + 8 * sizeof(float);
    ...
    attributeDescriptors[3].offset = offsetof(ObjectVertex, model) + 12 * sizeof(float);
